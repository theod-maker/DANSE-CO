...
const PricingCard = ({ label, price, detail, highlight, index }: PricingRowContent & { index: number }) => {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, x: index % 2 === 0 ? -30 : 30, y: 10 }} animate={isInView ? { opacity: 1, x: 0, y: 0 } : {}} whileHover={{ y: -4, transition: { duration: 0.25 } }} transition={{ duration: 0.7, delay: index * 0.1, ease: EASING }} className={`group liquid-glass rounded-3xl p-7 flex items-center justify-between gap-6 relative overflow-hidden ${highlight ? 'ring-1 ring-[#6C5CA8]/25' : ''}`}>
      <div className="card-shimmer-layer" />
      {highlight && <div className="absolute left-0 inset-y-0 w-1 bg-gradient-to-b from-[#6C5CA8]/40 via-[#6C5CA8]/70 to-[#6C5CA8]/40 rounded-full" />}
      <div className="flex-1 relative z-10">
        <div className="flex items-center gap-3 mb-1.5">
          <p className="text-[#18102E] text-lg font-medium">{label}</p>
          {highlight && <motion.span initial={{ opacity: 0, scale: 0.8 }} animate={isInView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.4, delay: 0.2 + index * 0.1 }} className="bg-[#EDEAF6] rounded-full px-3 py-0.5 text-xs text-[#6C5CA8] font-medium">Populaire</motion.span>}
        </div>
        <p className="text-[#18102E]/40 text-sm">{detail}</p>
      </div>
      <div style={{ fontFamily: "'Instrument Serif', serif" }} className="text-[#6C5CA8] text-3xl md:text-4xl tracking-tight whitespace-nowrap relative z-10">{price}</div>
    </motion.div>
  )
}
  ...
  return (
    <div className="min-h-screen overflow-x-hidden">
      ...
      <main className="max-w-6xl mx-auto px-6 pt-40 pb-32 relative">
        <div className="absolute pointer-events-none [animation:orb-drift-slow_18s_ease-in-out_infinite]" style={{ width: 500, height: 500, top: '-80px', right: '-150px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(107,33,168,0.05) 0%, transparent 70%)', filter: 'blur(60px)' }} />
        <div ref={headerRef} className="mb-20 relative z-10">
          <motion.p initial={{ opacity: 0, y: 20 }} animate={headerInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="text-[#18102E]/40 text-xs tracking-widest uppercase font-ui mb-6">SAISON {pricingData.season}</motion.p>
