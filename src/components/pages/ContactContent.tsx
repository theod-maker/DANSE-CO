  ...
  return (
    <div className="min-h-screen overflow-x-hidden">
      ...
      <main className="max-w-6xl mx-auto px-6 pt-40 pb-32 relative">
        <div className="absolute pointer-events-none [animation:orb-drift-alt_14s_ease-in-out_infinite]" style={{ width: 450, height: 450, top: '-60px', left: '-120px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(107,33,168,0.05) 0%, transparent 70%)', filter: 'blur(55px)' }} />
        <div ref={headerRef} className="mb-20 relative z-10">
          <motion.p initial={{ opacity: 0, y: 20 }} animate={headerInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="text-[#18102E]/40 text-xs tracking-widest uppercase font-ui mb-6">NOUS ÉCRIRE</motion.p>
        ...
        <div className="grid md:grid-cols-2 gap-8 items-start relative z-10">
          ...
          <motion.div ref={infoRef} initial={{ opacity: 0, x: 30 }} animate={infoInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.1, ease: EASING }} className="liquid-glass rounded-3xl p-8 flex flex-col gap-8 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-col gap-5">
                {contactItems.map((item, index) => (
                  <motion.div key={index} initial={{ opacity: 0, x: 20 }} animate={infoInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.2 + index * 0.12, ease: EASING }} className="flex items-start gap-4">
                    <div className="bg-[#EDEAF6] rounded-full p-2.5 text-[#6C5CA8] shrink-0">{item.icon}</div>
                    <div>
                      <p className="text-[#18102E]/40 text-xs mb-1">{item.label}</p>
                      {item.href ? (
                        <a href={item.href} className="text-[#18102E]/75 text-sm hover:text-[#6C5CA8] transition-colors">{item.value}</a>
                      ) : (
            ...
            <motion.div initial={{ scaleX: 0 }} animate={infoInView ? { scaleX: 1 } : {}} transition={{ duration: 0.7, delay: 0.6, ease: EASING }} className="w-full h-px bg-[#18102E]/8 origin-left relative z-10" />
            <div className="relative z-10">
              <p className="text-[#18102E]/40 text-xs tracking-widest uppercase font-ui mb-4">RÉSEAUX</p>
              <div className="flex gap-3">
                {siteInfo.instagramUrl && (
                  <motion.a href={siteInfo.instagramUrl} target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.1, rotate: 5 }} whileTap={{ scale: 0.95 }} className="bg-[#EDEAF6] rounded-full p-3 text-[#6C5CA8] hover:bg-[#6C5CA8] hover:text-white transition-colors">
                    <InstagramIcon size={18} />
                  </motion.a>
                )}
                {siteInfo.facebookUrl && (
                  <motion.a href={siteInfo.facebookUrl} target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.1, rotate: 5 }} whileTap={{ scale: 0.95 }} className="bg-[#EDEAF6] rounded-full p-3 text-[#6C5CA8] hover:bg-[#6C5CA8] hover:text-white transition-colors">
              ...
              {venues.map((venue) => (
                <div key={venue._id} className="liquid-glass rounded-3xl overflow-hidden">
                  ...
                  <div className="px-5 py-4">
                    ...
                    {venue.googleMapsUrl && (
                      <a href={venue.googleMapsUrl} target="_blank" rel="noopener noreferrer" className="text-[#6C5CA8] text-xs hover:underline mt-2 inline-block">Ouvrir dans Google Maps →</a>
                    )}
                  </div>
                </div>
              ))}
